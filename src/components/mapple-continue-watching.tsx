...
// Types matching the Mapple localStorage structure
interface MappleProgressDetails {
  watched: number;
  duration: number;
}

interface MappleEpisodeProgress {
  season: string;
  episode: string;
  progress: MappleProgressDetails;
  last_updated: number;
}

interface MappleMedia {
  id: string;
  type: MediaType;
  title: string;
  poster_path: string;
  ...
  show_progress?: Record<string, MappleEpisodeProgress>;
}
...
const ContinueWatchingCard = ({ item }: { item: MappleMedia }) => {
...
export default function MappleContinueWatching() {
  const [items, setItems] = React.useState<MappleMedia[]>([]);

  const loadProgress = React.useCallback(() => {
    try {
      const progressData = localStorage.getItem('mapplePlayerProgress');
      if (progressData) {
        const parsedData: Record<string, MappleMedia> = JSON.parse(progressData);
        const sortedItems = Object.values(parsedData).sort((a, b) => b.last_updated - a.last_updated);
        setItems(sortedItems);
      }
    } catch (error) {
      console.error("Failed to parse Mapple progress:", error);
      setItems([]);
    }
  }, []);
  ...
  React.useEffect(() => {
    loadProgress();
    window.addEventListener('mapple-progress-change', loadProgress);
    return () => {
      window.removeEventListener('mapple-progress-change', loadProgress);
    };
  }, [loadProgress]);
